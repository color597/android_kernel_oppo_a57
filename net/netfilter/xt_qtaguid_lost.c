...
static void lost_node_tree_insert(struct lost_node *data, struct rb_root *root)
{
...
static void ipv4_stat_tree_insert(struct ipv4_lost_stat *stat, struct rb_root *root)
{
	lost_node_tree_insert(&stat->ln, root);
}

static void ipv6_stat_tree_insert(struct ipv6_lost_stat *stat, struct rb_root *root)
{
	lost_node_tree_insert(&stat->ln, root);
}
			...
			struct iface_stat *iface_entry, const enum ifs_tx_rx direction)
{
	...
	if (!node) {
		...
		ipv6_stat_tree_insert(ipv6_ls, &iface_entry->ipv6_lost_stat_tree);
		node = &ipv6_ls->ln;
	} else {
			...
			struct iface_stat *iface_entry, const enum ifs_tx_rx direction)
{
	...
	if (!node) {
		...
		ipv4_stat_tree_insert(ipv4_ls, &iface_entry->ipv4_lost_stat_tree);
		node = &ipv4_ls->ln;
	} else {
...
static void __init init_lost_stat_params(void)
{
	...
	lost_stat_params[LS_CLOSE_ENABLE] = 0;
...
static int __init init_lost_stat(void)
{
	...
	LS_DEBUG("[lost_stat] kernel obj name %s\n", qtaguid_module_kobj->name);
