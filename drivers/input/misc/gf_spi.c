...
#ifdef VENDOR_EDIT
//LiuPing@Phone.BSP.Sensor, 2016/08/10, add for compatibility fp.
...
static int g_disable_fb_test = 1;
...
{
	struct gf_dev *gf_dev = &gf;
       printk(KERN_EMERG "lycan test %s %d\n", __func__, __LINE__);
#ifdef GF_FASYNC
	if (gf_dev->async)
...
{
    if (g_disable_fb_test ) {
        printk(KERN_EMERG "lycan test ignore %s \n", __func__);
    }
    else {
        struct gf_dev *gf_dev;
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
	...
	__set_bit(GF_CLICK, gf_dev->input->keybit);
...
#ifdef VENDOR_EDIT
//LiuPing@Phone.BSP.Sensor, 2016/08/10, add for compatibility fp.
        if(FP_GOODIX != get_fpsensor_type())
        {
        	pr_err("found not goodix sensor\n");
            status = -1;
        	goto error;
        }
       pr_info("found goodix sensor\n");
#endif /*VENDOR_EDIT*/
