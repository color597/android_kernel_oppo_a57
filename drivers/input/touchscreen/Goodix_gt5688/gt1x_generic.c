...
#define GOODIX_PINCTRL_STATE_LOW  "gt1x_int_low"
#define GOODIX_PINCTRL_STATE_HIGH "gt1x_int_high"
#define GOODIX_PINCTRL_STATE_NOPULL  "gt1x_int_nopull"
...
extern struct Coordinate Point_start;
extern struct Coordinate Point_end;
extern struct Coordinate Point_1st;
extern struct Coordinate Point_2nd;
extern struct Coordinate Point_3rd;
extern struct Coordinate Point_4th;
extern uint32_t clockwise;
extern uint32_t gesture_upload;
extern atomic_t double_enable;
extern atomic_t is_in_suspend;
extern struct mutex i2c_access;
extern struct mutex gt_suspend_lock;
...
struct gtp_pinctrl_info {
	...
	struct pinctrl_state *gpio_state_high;
	struct pinctrl_state *gpio_state_low;
	struct pinctrl_state *gpio_state_nopull;
};
static struct gtp_pinctrl_info gt1x_pctrl;
...
int gpio_set_int_nopull(void);
int gpio_set_int_level(bool level);
...
extern irqreturn_t gt1x_ts_irq_handler(int irq, void *dev_id);
...
static int gt1x_read_data(int type, char * output){
	...
	gt1x_rawdiff_mode=1;
	msleep(20);
	gt1x_i2c_write(0x814E, &end_cmd, 1);
	GTP_INFO("%d,%s\n",__LINE__,__func__);
	// wait for data ready
	while(i++ < 100){
		ret = gt1x_i2c_read(0x814E, buffer, 1);
		if(!ret && ((buffer[0]&0x80)==0x80)){
			GTP_INFO("Data ready OK");
			break;
		}
		msleep(20);
	}
	...
	for(i = 0; i < DRIVER_NUM; i++){
		...
		for(j = 0; j < SENSOR_NUM; j++){
				p += sprintf(p, "%d\t", (s16)(((s16)buffer[i * len + j * 2] << 8) + buffer[i * len + j * 2 + 1]));
...
s32 gt1x_init_panel(void)
{
	...
	//read tp fw and factory information
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				#ifdef SUPPORT_EDGE_PROTECT
				if(limit_enable) {
					if(input_x > left_x2 && input_x < right_x2) {		//area 3
						if(1 == in_which_area) {
							//free fingers
							//GTP_INFO("area 3");
						...
						in_which_area = 3;
						//GTP_INFO("in_which_area is %d", in_which_area);
					}

					if(input_x < left_x1 && input_x > right_x1) {			//area 2
						if(1 == in_which_area) 
							in_which_area = 2;
						//GTP_INFO("in_which_area is %d", in_which_area);
					}

					if(input_x < left_x1 ||input_x > right_x1 ) {			//area 1
						if(2 == in_which_area) {
							in_which_area =1;
...
static ssize_t tp_double_write_func(struct file *file, const char __user *buffer, size_t count,loff_t *ppos)
{
	...
	mutex_unlock(&gt_suspend_lock);	//aaa
...
static ssize_t limit_area_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if(copy_from_user(buf, buffer, count))
	{
		GTP_ERROR(KERN_INFO "%s: read proc input error.\n", __func__);
		return count;
	}
	...
	limit_area = temp;
	left_x1 = (limit_area*720)/100;
	right_x1 = gt1x_abs_x_max - left_x1;
	left_x2 = 2 * left_x1;
	right_x2 = gt1x_abs_x_max -2*left_x1;
	GTP_INFO("limit_area = %d left_x1 = %d right_x1 = %d left_x2 =%d right_x2 = %d\n", limit_area, left_x1, right_x1, left_x2, right_x2);
	return count;
}
...
static int init_goodix_proc(void)
{
...
#ifdef SUPPORT_EDGE_PROTECT
	prEntry_dtap = proc_create("oppo_tp_limit_enable", 0664, prEntry_tp, &proc_limit_enable);
	if(prEntry_dtap == NULL)
	{
		ret = -ENOMEM;
		GTP_ERROR(KERN_INFO"init_gt1x_proc: Couldn't create proc entry\n");
	...
	prEntry_dtap = proc_create("oppo_tp_limit_area", 0664, prEntry_tp, &proc_limit_area);
	if(prEntry_dtap == NULL)
	{
		ret = -ENOMEM;
		GTP_ERROR(KERN_INFO"init_gt1x_proc: Couldn't create proc entry\n");
...
int gpio_set_int_level(bool level)
{
	...
	if(level) {
		ret = pinctrl_select_state(gt1x_pctrl.pinctrl, gt1x_pctrl.gpio_state_high);
		if (ret)
			GTP_ERROR("cannot set pin to high state");
	} else {
		ret = pinctrl_select_state(gt1x_pctrl.pinctrl, gt1x_pctrl.gpio_state_low);
		if (ret)
			GTP_ERROR("cannot set pin to low state");
	}
	return ret;
}
...
{
	int ret = -1;
	ret = pinctrl_select_state(gt1x_pctrl.pinctrl, gt1x_pctrl.gpio_state_nopull);
	if (ret)
		GTP_ERROR("cannot set pin to input state");
...
static int gtp_pinctrl_init(struct device *dev)
{
	gt1x_pctrl.pinctrl= devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(gt1x_pctrl.pinctrl)) {
                   GTP_ERROR("Getting pinctrl handle failed");
                   return -EINVAL;
	}

	gt1x_pctrl.gpio_state_high = pinctrl_lookup_state( gt1x_pctrl.pinctrl, GOODIX_PINCTRL_STATE_HIGH);
	if (IS_ERR_OR_NULL(gt1x_pctrl.gpio_state_high)) {
		GTP_ERROR ("Failed to get the high state pinctrl handle");
		return -EINVAL;
	}

	gt1x_pctrl.gpio_state_low = pinctrl_lookup_state(gt1x_pctrl.pinctrl, GOODIX_PINCTRL_STATE_LOW);
	if (IS_ERR_OR_NULL(gt1x_pctrl.gpio_state_low)) {
		GTP_ERROR(" Failed to get the low state pinctrl handle");
		return -EINVAL;
	}

	gt1x_pctrl.gpio_state_nopull = pinctrl_lookup_state(gt1x_pctrl.pinctrl, GOODIX_PINCTRL_STATE_NOPULL); 
	if (IS_ERR_OR_NULL(gt1x_pctrl.gpio_state_nopull)) {
		GTP_ERROR("Failed to get the input state pinctrl handle");
		return -EINVAL;
	}
...
s32 gt1x_init(void)
{
	...
	ret = gtp_pinctrl_init(&gt1x_i2c_client->dev);
	if(ret != 0) {
		GTP_ERROR("Pinctrl Init failed, exit");
		return -1;
	}
    /* power on */
	gt1x_power_switch(SWITCH_ON);
	...
	if (ret) {
		GTP_ERROR("Init failed, quit init, retry is %d, ret is %d", retry, ret);
		return -1;
	}
	...
	#ifdef SUPPORT_EDGE_PROTECT
	left_x1 = (limit_area * gt1x_abs_x_max)/100;
	right_x1 = gt1x_abs_x_max - left_x1;
	left_x2 = 2 * left_x1;
	right_x2 = gt1x_abs_x_max - 2 * left_x1;
	#endif

	ret = init_goodix_proc();
