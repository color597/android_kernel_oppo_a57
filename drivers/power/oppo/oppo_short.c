...
void oppo_chg_short_c_hw_check(struct oppo_chg_chip *chip)
{
	...
	bool shortc_gpio_status = chip->short_c_batt.shortc_gpio_status;

    if (!chip || !chip->chg_ops->get_shortc_hw_gpio_status) {
        return;
    }

	if (chip->chg_ops->get_shortc_hw_gpio_status(chip) == 0) {
...
void oppo_chg_short_c_battery_check(struct oppo_chg_chip *chip)
{
	...
	if ((chip->chg_ops->get_dyna_aicl_result)
			&& (chip->chg_ops->get_dyna_aicl_result(chip) < INPUT_LIMIT_MA_THRESHOLD)) {
		short_debug("get_dyna_aicl_result[%d]\n", chip->chg_ops->get_dyna_aicl_result(chip));
		goto cv_reset;
	}
