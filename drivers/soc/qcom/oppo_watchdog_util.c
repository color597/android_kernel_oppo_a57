...
void dump_cpu_online_mask()
{
	...
	cpulist_scnprintf(alive_mask_buf, MASK_SIZE, cpu_online_mask);
...
void get_cpu_ping_mask(cpumask_t *pmask)
{
	...
	printk(KERN_INFO "[wdog_util]cpu online mask: 0x%lx; ping mask: 0x%lx; irqs since last: %u\n",
		*cpumask_bits(pmask), *cpumask_bits(cpu_online_mask), o_irq_counter.all_irqs_delta);
}
...
void dump_wdog_cpu(struct task_struct *w_task)
{
	...
	/* trust panic can dump all cpu
	printk(KERN_EMERG "Task dump of all online cpus:\n");
	for_each_cpu(work_cpu, cpu_online_mask)
