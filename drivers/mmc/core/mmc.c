...
static int mmc_select_hs(struct mmc_card *card)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			   ...
			   card->ext_csd.generic_cmd6_time,
#ifndef VENDOR_EDIT	//Fanhong.Kong@ProDrv.CHG,add 2016/12/06 for cmd6 and cmd13 timeout 		   
//			   true, true, true);
//	if (!err)	
#else/*VENDOR_EDIT*/
			   true, false, true);
...
#ifdef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2016/12/06 for cmd6 and cmd13 timeout 
...
//	if (!err)	
	...
	if (!err){
			...
			err = mmc_switch_status(card);
			if ( err == -EILSEQ ) {
				pr_warn("%s %s ignore crc error from mmc_switch_status\n",
...
#ifndef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2017/04/21 for cmd6 SLC at suspend/resume
...
#ifdef VENDOR_EDIT//Fanhong.Kong@ProDrv.CHG,add 2016/11/16 for MICRON 3224 to fixup CQSSC1 to 0x7003
