...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
	ci->under_dcim = pi->under_dcim;
	if (ci->under_dcim && !ci->dcim_uid) {
		ci->dcim_uid = current_uid().val;
	} else {
		ci->dcim_uid = 0;
	}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
			else if (!strcasecmp(dentry->d_name.name, "DCIM")) {
				info->perm = PERM_DCIM;
			}
#endif /* VENDOR_EDIT */
			break;
...
//Jiemin.Zhu@PSW.Android.SdardFs, 2017/12/12, Add for sdcardfs delete dcim record
		case PERM_DCIM:
			if (sbi->options.multi_user && !strcasecmp(dentry->d_name.name, "Camera")) {
				info->perm = PERM_DCIM;
				info->under_dcim = true;
			} else if (sbi->options.multi_user && !strcasecmp(dentry->d_name.name, "Screenshots")) {
				info->perm = PERM_DCIM;
				info->under_dcim = true;
			}
			break;
